...
function renderLearningUser() {
  const loginButton = document.querySelector('.nav-cta');
  const user = JSON.parse(localStorage.getItem('skillNestUser') || 'null');
  if (!loginButton || !user) return;
  loginButton.outerHTML = `
    <div class="learning-user-profile show">
      <span class="learning-user-icon">👤</span>
      <span class="learning-user-name">${user.name || user.email || 'User'}</span>
    </div>
  `;
}
...
if (loginForm) {
  loginForm.addEventListener('submit', (event) => {
    ...
    const nameInput = document.getElementById('learningUserName');
    ...
    if (alertBox) {
      ...
      alertBox.textContent = 'Login successful. Your profile is now shown in the navbar.';
    }
    setTimeout(() => {
